...
public extension AsyncType where Value: ResultType {
    ...
    /// Returns self
    public func onSuccess(context: ExecutionContextType = DefaultThreadingModel(), callback: Value.Value -> Void) -> Self {
    ...
    /// If no context is given, the behavior is defined by the default threading model (see README.md)
    /// Returns self
    public func onFailure(context: ExecutionContextType = DefaultThreadingModel(), callback: Value.Error -> Void) -> Self {
        self.onComplete(context) { result in
    ...
    public func flatMap<U>(context: ExecutionContextType, f: Value.Value -> Future<U, Value.Error>) -> Future<U, Value.Error> {
        return map(context, f: f).flatten()
    }
    
    /// See `flatMap<U>(context c: ExecutionContextType, f: T -> Future<U, E>) -> Future<U, E>`
    ...
    /// `flatMap<U>(context c: ExecutionContextType, f: T -> Future<U>) -> Future<U>`
    public func flatMap<U>(context: ExecutionContextType, f: Value.Value -> Result<U, Value.Error>) -> Future<U, Value.Error> {
        return self.flatMap(context) { value in
            return Future<U, Value.Error>(result: f(value))
        }
    }
    
    /// See `flatMap<U>(context c: ExecutionContextType, f: T -> Result<U, E>) -> Future<U, E>`
    /// The given closure is executed according to the default threading model (see README.md)
    public func flatMap<U>(f: Value.Value -> Result<U, Value.Error>) -> Future<U, Value.Error> {
        return flatMap(DefaultThreadingModel(), f: f)
    }
    
    /// See `map<U>(context c: ExecutionContextType, f: (T) -> U) -> Future<U>`
    /// The given closure is executed according to the default threading model (see README.md)
    ...
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func map<U>(context: ExecutionContextType, f: Value.Value -> U) -> Future<U, Value.Error> {
    ...
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func recover(context c: ExecutionContextType = DefaultThreadingModel(), task: Value.Error -> Value.Value) -> Future<Value.Value, NoError> {
    ...
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func recoverWith<E1: ErrorType>(context c: ExecutionContextType = DefaultThreadingModel(), task: Value.Error -> Future<Value.Value, E1>) -> Future<Value.Value, E1> {
    ...
    /// See `mapError<E1>(context c: ExecutionContextType, f: E -> E1) -> Future<T, E1>`
    /// The given closure is executed according to the default threading model (see README.md)
    ...
    public func mapError<E1: ErrorType>(context: ExecutionContextType, f: Value.Error -> E1) -> Future<Value.Value, E1> {
        let res = Future<Value.Value, E1>()
    ...
    public func andThen(context c: ExecutionContextType = DefaultThreadingModel(), callback: Self.Value -> Void) -> Self {
    ...
    public func zip<U>(that: Future<U, Value.Error>) -> Future<(Value.Value,U), Value.Error> {
        return flatMap(immediate) { thisVal -> Future<(Value.Value,U), Value.Error> in
    ...
    public func filter(p: Value.Value -> Bool) -> Future<Value.Value, BrightFuturesError<Value.Error>> {
        return self.mapError(immediate) { error -> BrightFuturesError<Value.Error> in
            return BrightFuturesError(external: error)
        }.flatMap(immediate) { value -> Result<Value.Value, BrightFuturesError<Value.Error>> in
            if p(value) {
...
public extension AsyncType where Value: ResultType, Value.Error == NoError {
    ...
    public func promoteError<E: ErrorType>() -> Future<Value.Value, E> {
        return mapError(immediate) { $0 as! E } // future will never fail, so this map block will never get called
    }
}
...
public extension AsyncType where Value: ResultType, Value.Error == BrightFuturesError<NoError> {
    ...
    public func promoteError<E: ErrorType>() -> Future<Value.Value, BrightFuturesError<E>> {
        return mapError(immediate) { err in
            switch err {
            case .NoSuchElement:
...
public extension AsyncType where Value: ResultType, Value.Value == NoValue {
    ...
    public func promoteValue<T>() -> Future<T, Value.Error> {
        return map(immediate) { $0 as! T } // future will never succeed, so this map block will never get called
    }
}
