...
/// If the given closure throws, returns an error
public func future<T>(context: ExecutionContextType, task: () throws -> T) -> Future<T, AnyError> {
...
public extension AsyncType where Value: ResultType, Value.Error == AnyError {
    ...
    /// Returns self
    public func onFailure(context: ExecutionContextType = DefaultThreadingModel(), callback: ErrorType -> Void) -> Self {
    ...
    public func map<U>(context: ExecutionContextType, f: Value.Value throws -> U) -> Future<U, Value.Error> {
    ...
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func recover(context c: ExecutionContextType = DefaultThreadingModel(), task: ErrorType -> Value.Value) -> Future<Value.Value, NoError> {
    ...
    /// If the given closure throws, returns an error
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func recover(context c: ExecutionContextType = DefaultThreadingModel(), task: ErrorType throws -> Value.Value) -> Future<Value.Value, AnyError> {
    ...
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func recoverWith<E1: ErrorType>(context c: ExecutionContextType = DefaultThreadingModel(), task: ErrorType -> Future<Value.Value, E1>) -> Future<Value.Value, E1> {
        return recoverWith(context: c) { (error:Value.Error) -> Future<Value.Value, E1> in
            task(error.cause)
        }
    }
    
    /// See `mapError<E1>(context c: ExecutionContextType, f: ErrorType -> E1) -> Future<T, E1>`
    /// The given closure is executed according to the default threading model (see README.md)
    ...
    public func mapError<E1: ErrorType>(context: ExecutionContextType, f: ErrorType -> E1) -> Future<Value.Value, E1> {
...
public extension AsyncType where Value: ResultType {
    ...
    /// If the given closure throws, returns an error
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func map<U>(context: ExecutionContextType, f: Value.Value throws -> U) -> Future<U, AnyError> {
    ...
    /// If the given closure throws, returns an error
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func recover(context c: ExecutionContextType = DefaultThreadingModel(), task: Value.Error throws -> Value.Value) -> Future<Value.Value, AnyError> {
        return self.recoverWith(context: c) { error -> Future<Value.Value, AnyError> in
            do {
                return Future<Value.Value, AnyError>(value: try task(error))
            } catch let e {
